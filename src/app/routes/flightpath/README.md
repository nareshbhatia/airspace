...
| **Step 4** DroneServiceImpl       | `interval` + `takeUntil`. Inject `DroneStoreApi` from provider. Batch via `_updateDrones`. Verify DevTools shows `_updateDrones` at tick rate.                                                                                                                   | Injected store bridge, zero React imports.                                                         |
...
8. Redux DevTools shows `_updateDrones` and `_tick` firing at 10 Hz.
